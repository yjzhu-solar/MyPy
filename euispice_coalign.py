...
def create_syn_raster(spice_file, eui_files, spice_window,
    ...
    if save_filename is not None:
        np.savez_compressed(save_filename, eui_syn_raster_image=eui_syn_raster_image)
...
def calculate_eui_spice_shift(spice_file, eui_files, spice_window, eui_syn_raster_image,
    ...
    if isinstance(eui_syn_raster_image, str):
        eui_syn_raster_image = np.load(eui_syn_raster_image)['eui_syn_raster_image']
    ...
    spice_cdelt1 = spice_window.meta['CDELT1']
    ...
    if rotation:
        rot_angles = np.deg2rad(np.linspace(-1,1,11))
    else:
        rot_angles = [0]
    ...
    for ii, rot_matrix in enumerate(rotation_matrices):
        rotation_matrix_inv = np.linalg.inv(rot_matrix)
        xy_grid_prime = np.flip(np.indices(spice_int_img.data.shape),axis=0)
        rot_center = np.array([spice_window.meta['CRPIX1'] - 1, spice_window.meta['CRPIX2'] - 1])
        ...
        spice_int_img_rotated = ndimage.map_coordinates(spice_int_img, yx_grid_0, order=1)



        spice_int_img_cut = spice_int_img_rotated[spice_ny//4:3*spice_ny//4, spice_nx//4:3*spice_nx//4]
    ...
    shift_reference_world_coord = spice_wcs.pixel_to_world(xshift_optimal, yshift_optimal, 0)[0]
    reference_pixel_world_coord = spice_wcs.pixel_to_world(spice_nx//4,spice_ny//4,0)[0]
    ...
    eui_syn_raster_map = sunpy.map.Map(eui_syn_raster_image, spice_wcs)
    eui_syn_raster_map.plot_settings['aspect'] = eui_syn_raster_map.scale.axis2/eui_syn_raster_map.scale.axis1
    spice_int_map = sunpy.map.Map(spice_int_img, spice_wcs)
...
def save_new_spice_file(spice_file, crval1, crval2, rotation_matrix, outdir=None, filename=None):
    with fits.open(spice_file) as hduls:
        for hdul in hduls:
            if 'CRVAL1' in hdul.header.keys():
                hdul.header = update_header(hdul.header, crval1, crval2, rotation_matrix)
        if outdir is None:
...
def update_header(hdr, crval1, crval2, rotation_matrix):
    hdr_new = hdr.copy()
